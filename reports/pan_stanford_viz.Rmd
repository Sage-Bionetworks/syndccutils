---
title: "Stanford CSBC Project Summary"
output:
  html_notebook:
    code_folding: hide
...
####Assays by patient and project

```{r, message=FALSE, echo=FALSE}
assays_by_patient <- fv_df %>% 
  group_by(individualID, assay, projectName) %>%
  tally() %>% 
  ungroup() %>%
  mutate(individualID = fct_rev(individualID)) %>% 
  mutate(projectName = fct_rev(projectName)) %>% 
  ggplot(aes(individualID)) +
    geom_bar(aes(fill = assay), alpha = 0.8, colour = "white",
             position = position_stack(reverse = TRUE)) +
