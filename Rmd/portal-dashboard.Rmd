...
### Data upload frequency over time (`ggridges`)
...
### Data upload over time (`streamgraph`)
...
```{r}
fillRow(
    streamgraphOutput("files_per_month")    
)
...
output$center_metadata <- function() {
    ...
    # selected_project
    knitr::kable(center_data_df, "html", escape = FALSE)
}
...
display_vars <- names(center_summary_df) %>% 
    setdiff(setdiff(project_vars, c("projectId", "name_project")))
output$center_summary <- DT::renderDT({
    center_summary_df %>% 
        select(one_of(display_vars)) %>% 
